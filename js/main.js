...
$("input").on('focus', function () {
    "use strict";
    $(this).parent().children('div.bor').animate({
        width: '100%',
        left: '0px'
    });
}).on('blur', function () {
...
$('div.controls span').on("click", function () {
    ...
    if ($(this).hasClass('first')) {
        $("div.controls div.after").css({
            "right": "50%",
            "width": "50%"
        });
        $("div.slide div.inner").animate({
            "left": "0%"
        });
    } else {
        $("div.controls div.after").css({
...
$("div.following div.cir").on("mouseenter", function () {
    ...
    if ($(this).data('open') === false) {
        $(this).children("div.after").animate({
            "top": "-50%"
        });
        $(this).children("div.before").animate({
            "bottom": "-50%"
        });
        $(this).siblings("span.outter").animate({
            "opacity": "1"
        }, 1000);
        $(this).data('open', true);
    } else {
...
function OpenProfile() {
    ...
    $("section." + $("aside.left-side ul li.profileLink").data("dir")).css({
        "display": "block"
    }).animate({
        "opacity": "1"
    });
...
function OpenDash() {
    ...
    $("section." + $("aside.left-side ul li.dash").data("dir")).css({
        "display": "block"
    }).animate({
        "opacity": "1"
    });
...
$('.open-panel').on("click", function () {
    ...
    if ($(this).parent().siblings(dataTargetClassDirection).hasClass('hidden')) {
        $(this).parent().siblings(dataTargetClassDirection).animate({
            "opacity": "0.6"
        }, speedOfSwitchPanel).removeClass('hidden');
    } else {
        $(this).parent().siblings(dataTargetClassDirection).animate({
            "opacity": "0"
        }, speedOfSwitchPanel).addClass('hidden');
    } //end of else 
}); //end of toggle panel
/*
...
$("div.session-panel").on("mouseenter", function () {
    "use strict";
    $(this).children(".enter").animate({
        display: "block",
        opacity: "1"
    });
    $(this).children(".bottom-panel").animate({
        bottom: "0px"
    });
});
...
function calcDataDeff(startDate, endDate) {
    var deff = endDate.getTime() - startDate.getTime();
    var min = deff / (60 * 60) * 60;
    //alert(startDate.getTime()- endDate.getTime());
    alert(deff);
}

//calcDataDeff(new Date(""), new Date(""));
$('#openProfile').on("click", function () {
    "use strict";
    OpenProfile();
});

function niceScroll(eve) {
    $(eve).niceScroll({
        cursorcolor: "#fff", // change cursor color in hex
        cursoropacitymin: 0, // change opacity when cursor is inactive (scrollabar "hidden" state), range from 1 to 0
        cursoropacitymax: 1, // change opacity when cursor is active (scrollabar "visible" state), range from 1 to 0
        cursorwidth: "5px", // cursor width in pixel (you can also write "5px")
        cursorborder: "1px solid #09ad92", // css definition for cursor border
